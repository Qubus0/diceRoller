...
static func base_dice_parser(dice_string: String, group_id: String)->Dictionary:
	...
	dh.dice_error(int(dice_side) in dice_types, "Invalid dice type.\nValid types: " + String(dice_types), rolling_rules)
	if dice_side!=null:
...
#		dh.dice_error(drop_count != null, "Malformed dice string: No drop count provided", rolling_rules) # never triggered due to * in regex
	...
	dh.dice_error(rolling_rules.dice_count > 0, "Invalid dice: Can't roll less than one die", rolling_rules)
	dh.dice_error(rolling_rules.dice_count <= 0 or rolling_rules.drop_dice < rolling_rules.dice_count, "Invalid dice: Can't drop all the dice you have", rolling_rules)
